...
export default function TicketResolveControl({ ticket, userId, summary, onSummaryChange, pending, resolving, onResolve }) {
  ...
  return <section aria-labelledby="resolve-heading" className="min-w-0 rounded-2xl border border-teal-200 bg-white p-4 sm:p-6">
    ...
    <form onSubmit={submit} noValidate className="mt-3 space-y-3">
      <label htmlFor="resolution-summary" className="block text-sm font-semibold">Resolution summary</label>
      <p id="resolution-help" className="text-sm text-slate-600">Record how the issue was resolved (10–5,000 characters).</p>
      ...
      {confirming ? <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button type="button" disabled={pending} className={buttonClass} onClick={onResolve}>{resolving ? 'Resolving...' : 'Confirm Resolve'}</button>
        </div>
      </div> : <button type="submit" disabled={pending} className={buttonClass}>Resolve Ticket</button>}
    </form>
  </section>
}
