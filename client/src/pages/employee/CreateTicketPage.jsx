...
// Phase 14.4 will supply options from an authoritative lookup API.
// No ticket category/priority lookup endpoints currently exist in the backend.
export default function CreateTicketPage() {
  return <CreateTicketForm />
}

export function CreateTicketForm({ categories = [], priorities = [] }) {
  ...
  const optionsAvailable = categories.length > 0 && priorities.length > 0
    ...
    <form ref={form} onSubmit={submit} noValidate className="mt-6 space-y-6" aria-busy={creating}>
      ...
      <fieldset disabled={creating} className="space-y-6">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {[['categoryId', 'Category', categories], ['priorityId', 'Priority', priorities]].map(([name, label, options]) => <div key={name}>
            ...
            <select id={name} name={name} value={values[name]} onChange={update} required disabled={!options.length} className={inputClass} aria-invalid={Boolean(errors[name])} aria-describedby={`${name}-error${!optionsAvailable ? ' selection-notice' : ''}`}>
              <option value="">{options.length ? `Select ${label.toLowerCase()}` : `${label} options unavailable`}</option>
      ...
      {!optionsAvailable && <p id="selection-notice" role="status" className="rounded-lg bg-slate-50 p-3 text-sm text-slate-600">Category and priority options are not available yet. Ticket submission will be available once these options are ready.</p>}
      <div className="flex flex-wrap items-center gap-4">
        <button type="submit" disabled={creating} className="rounded-lg bg-teal-800 px-5 py-3 text-sm font-semibold text-white hover:bg-teal-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-700 disabled:opacity-50">{creating ? 'Creating ticket...' : 'Create Ticket'}</button>
