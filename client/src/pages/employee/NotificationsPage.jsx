...
export default function NotificationsPage() {
  const { user } = useAuth()
  return <Notifications key={user?.id} />
}
function Notifications() {
  ...
  useEffect(() => {
    ...
    getNotifications({ page: request.page, signal: controller.signal }).then(data => {
  ...
  return <div className="space-y-5">
    ...
    <p className="text-slate-600">Updates about your support requests.</p>
    ...
    {current?.data && <>
      ...
      {!current.data.notifications.length ? <p className="rounded-xl border border-slate-200 bg-white p-6">You don't have any notifications yet.</p> : <ul className="space-y-3">{current.data.notifications.map(item => item.type === 'INTERNAL_NOTE' ? null : <li key={item.id} className={`min-w-0 rounded-xl border p-5 ${item.isRead ? 'border-slate-200 bg-white' : 'border-teal-200 bg-teal-50'}`}>
        <p className="text-xs font-semibold text-slate-600">{item.isRead ? 'Read' : 'Unread'} · {labels[item.type] || 'Support update'}</p>
        ...
        <div className="mt-3 flex flex-wrap gap-3">
          {item.ticketId != null && /^[1-9]\d*$/.test(String(item.ticketId)) && <Link className={buttonClass} to={`/employee/tickets/${encodeURIComponent(item.ticketId)}`}>View Ticket</Link>}
