...
const invalid = [
  {}, [], null, { name: "" }, { name: " " }, { name: "a".repeat(101) },
...
test("service validates before SQL and normalizes optional descriptions with injected connections", async () => {
  ...
  for (const value of invalid) await assert.rejects(service.createCategory(value, 1, noQuery), { statusCode: 422 });
  for (const description of [undefined, null, "   ", "  Details  "]) {
    ...
    const db = { async query(sql, values) {
      ...
      if (calls === 2) { assert.deepEqual(values, ["Network", normalized, 1]); return [{ insertId: 7 }]; }
      assert.deepEqual(values, [7]);
      return [[{ id: 7, name: "Network", description: normalized, is_active: 1, created_at: "created", updated_at: "updated" }]];
    } };
    assert.deepEqual(await service.createCategory({ name: "  Network  ", description }, 1, db), {
      id: 7, name: "Network", description: normalized, isActive: true, createdAt: "created", updatedAt: "updated",
    });
    assert.equal(calls, 3);
  }
  const failure = new Error("connection failed");
  await assert.rejects(service.createCategory({ name: "Network" }, 1, { async query() { throw failure; } }), e => e === failure);
});
...
test("registered creation endpoint enforces ADMIN, validation, duplicate conflicts and mapped 201 responses", async (t) => {
  ...
  const base = `http://127.0.0.1:${server.address().port}/api/v1/tickets/admin`;
  ...
  assert.deepEqual(await result.json(), { success: true, message: "Ticket category created successfully",
  ...
  for (const duplicateMode of ["duplicate", "race"]) {
    ...
    assert.equal((await response.json()).message, "Ticket category already exists");
  }
