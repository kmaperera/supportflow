...
const service = require("../src/modules/tickets/ticketCategory.service");
const repository = require("../src/modules/tickets/ticketCategory.repository");
...
test("category edits preserve omitted fields, handle conflicts and make status changes idempotent", async (t) => {
  ...
  await assert.rejects(service.updateCategory(99, { name: "Valid" }, db), { statusCode: 404, message: "Ticket category not found" });
...
test("PATCH routes require ADMIN, validate bodies and return mapped categories without article SQL", async (t) => {
  ...
  const query = t.mock.method(pool, "query", async (sql, values) => {
    ...
    assert.match(sql, /ticket_categories/);
    ...
    if (sql.includes('WHERE is_active = TRUE')) return [row.is_active ? [{ id: row.id, name: row.name }] : []];
  ...
  const base = `http://127.0.0.1:${server.address().port}/api/v1/tickets/admin/categories`;
  ...
  const get = (path, actor = 1) => fetch(path, { headers: actor ? { authorization: `Bearer ${jwt.sign({}, process.env.JWT_ACCESS_SECRET, { subject: String(actor), expiresIn: '5m' })}` } : {} });
  for (const path of [base, base + '/5']) {
    assert.equal((await get(path, null)).status, 401);
    for (const actor of [2, 3]) assert.equal((await get(path, actor)).status, 403);
  }
  ...
  assert.equal((await get(base + '/abc')).status, 422);
  assert.equal((await get(base + '/99')).status, 404);
  ...
  assert.deepEqual(await response.json(), { success: true, message: "Ticket category updated successfully", data: {
  ...
  for (const active of [false, false, true, true]) {
    ...
    assert.equal(body.message, "Ticket category status updated successfully");
    assert.equal(body.data.category.isActive, active);
    const listed = await (await get(base)).json();
    assert.equal(listed.data.categories[0].isActive, active);
    assert.equal((await (await get(base + '/5')).json()).data.category.isActive, active);
    const lookup = base.replace('/admin/categories', '/categories');
    for (const actor of [1, 2, 3]) {
      const response = await get(lookup, actor);
      assert.equal(response.status, 200);
      assert.deepEqual((await response.json()).data.categories, active ? [{ id: 5, name: 'Connectivity' }] : []);
    }
  }
