...
async function create({ name, description = null, createdBy }, db = pool) {
  const [result] = await db.query(
    "INSERT INTO ticket_categories (name, description, created_by) VALUES (?, ?, ?)",
...
async function updateById(categoryId, { name, description = null }, db = pool) {
  const [result] = await db.query(
    "UPDATE ticket_categories SET name = ?, description = ? WHERE id = ?",
...
async function setActiveStatus(categoryId, isActive, db = pool) {
  const [result] = await db.query(
    "UPDATE ticket_categories SET is_active = ? WHERE id = ?",
    [isActive, categoryId]
  );
  return result.affectedRows;
}
