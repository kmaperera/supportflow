const repository = require("./ticketCategory.repository");
...
function normalizeCategory(values) {
  ...
  if (typeof values.name !== "string") {
    throw new ApiError(422, "Name must be a string between 1 and 100 characters");
  }
  const name = values.name.trim();
  if (Array.from(name).length < 1 || Array.from(name).length > 100) {
    throw new ApiError(422, "Name must be between 1 and 100 characters");
  }
...
async function createCategory(values, createdBy, db) {
  ...
  let id;
  try {
    id = await repository.create({ name, description, createdBy }, db);
  } catch (error) {
  ...
  if (!row) throw new ApiError(500, "Created Ticket category could not be retrieved");
...
async function requireCategory(categoryId, db) {
  ...
  if (!row) throw new ApiError(404, "Ticket category not found");
  return row;
}
...
async function updateCategory(categoryId, values, db) {
  ...
  try {
    ...
    if (error.code === "ER_DUP_ENTRY") throw new ApiError(409, "Ticket category already exists");
    throw error;
  }
...
async function getCategoryById(id, db) { validateCategoryId(id); return mapCategory(await requireCategory(id, db)); }
async function getCategories(db) { return (await repository.findAll(db)).map(mapCategory); }
module.exports = { getCategories, getCategoryById, createCategory, updateCategory, setCategoryActiveStatus };
