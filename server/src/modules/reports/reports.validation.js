...
function normalizeCategoryReportQuery(params = {}) {
  const allowed = ["startDate", "endDate", "priorityId", "technicianId"];
  if (!params || typeof params !== "object" || Array.isArray(params) || Object.keys(params).some(key => !allowed.includes(key))) invalid("Unsupported category report query parameter");
  return normalizeReportQuery(params, allowed).filters;
}
const categoryReportValidation = [query().custom(value => { normalizeCategoryReportQuery(value); return true; })];

function normalizePriorityReportQuery(params = {}) {
  const allowed = ["startDate", "endDate", "categoryId", "technicianId"];
  if (!params || typeof params !== "object" || Array.isArray(params) || Object.keys(params).some(key => !allowed.includes(key))) invalid("Unsupported priority report query parameter");
  return normalizeReportQuery(params, allowed).filters;
}
const priorityReportValidation = [query().custom(value => { normalizePriorityReportQuery(value); return true; })];

function normalizeStatusReportQuery(params = {}) {
  const allowed = ["startDate", "endDate", "categoryId", "priorityId", "technicianId"];
  if (!params || typeof params !== "object" || Array.isArray(params) || Object.keys(params).some(key => !allowed.includes(key))) invalid("Unsupported status report query parameter");
  return normalizeReportQuery(params, allowed).filters;
}
